...
**当前为可安装的开发候选，尚未通过发布验收。** 首次成图、多人原话、定向修改、恢复导出、失败保护和安装维护已实现。最新固定影像矩阵完成 18 次真实生成：15 次通过代理底线核验、3 次拒收，未观察到人物误交付；人工评分未填写。15 类行为通过，三条修改有成功证据，严格像素位移仍有失败。34 项本机测试与检查通过；跨平台结果及准确证据版本见 [最新修复与验证记录](docs/person-fidelity-fixes.md)。
...
当前固定候选为 `2a6156c5369a30c8ad28ec1e79533c4aa286671c`，仅在本地提交，尚未推送或公开发布。制品为 `dist/photo-dialogue-2a6156c.zip`，校验信息见 [候选清单](docs/candidate.json)。最后一项修改仅修复预览/下载的绝对路径，影像矩阵使用图像逻辑相同的 `ad0104e`。下面命令可从本地候选 checkout 构建并安装。使用已安装的合格 CPython 3.11–3.13，将 `python3.13` 替换为你选择的同一个解释器。
