...
- 故事书装帧改为逐册设计：图册稿显式保存照片与故事依据、配色、材质、字体、封面布局和翻阅节奏，阅读与视频采用同一设计；不再隐式套用样册绿色布封。

- 新增多图成册：Agent 关联照片并编排故事，先编排全书故事再逐页创作 Plog；导出可开合、翻页的实体书式 HTML，并本地录制 MP4/WebM 翻书动画（格式按浏览器能力选择）。
- 图册稿支持书名、文案、顺序与封面修改后另存版本；独立于单张 Plog 图片生成和版本记录。
