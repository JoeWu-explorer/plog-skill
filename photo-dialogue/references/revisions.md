...
成功候选需重查人物、中文、目标变化和未请求变化的重要决定。不要求逐像素不变：归一到同一尺寸后，约一两像素的栅格位置差、抗锯齿和细微纹理差异，若没有改变可见文字布局、整体光色、人物结构或关键动作，不单独判失败。“仅换文字”也不等于要求照片文件逐像素相同。未请求且非必要排版调整的字体风格/字号、阅读顺序、位置或整体光色变化仍判失败；裁人及实质人物变化独立判失败。用户明确要求逐像素一致或精确尺寸时不能套用这一容差，先说明生成式编辑无法保证的约束；不得静默放宽。失败与停止统一按 [运行约定](runtime.md#失败与清理) 处理。
