...
| 历史 Look 研究图 | prototypes/look-system/generated/PROVENANCE.md | PROJECT-DISPLAY；所有者已授权随项目公开，范围见 docs/image-rights.md |
...
运行依赖完整通知的本地副本在 `plog/notices/`，它们来自实际安装的锁定发行包；不改变其许可。

六图 CC0 不覆盖新成图、用户照片或其他图片。README examples 已登记逐文件来源、哈希与项目展示范围，尚未完成正式真人视觉审阅，也未授予通用图片再分发许可。参考项目只作为高层交付方式参考，未复制其代码、提示词或图像。未知或未获得所需授权的资产不得公开。当前图片来源核对与权利边界见 [展示图片的来源与使用范围](https://github.com/JoeWu-explorer/plog-skill/blob/main/docs/image-rights.md)。
