...
def summarize(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in evidence['attempts']:
        if row['sample'] not in SAMPLES or type(row['round']) is not int or row['round'] not in (1, 2) or type(row['attempt']) is not int or row['attempt'] < 1 or row['status'] not in STATUSES or type(row['misdelivered']) is not bool:
            raise ValueError('Invalid attempt identity, status or delivery flag.')
        misdelivered |= row['misdelivered']
        if row['attempt'] == 1:
            key = (row['sample'], row['round'])
            if key in first:
                raise ValueError('Duplicate first attempt; retain retries as distinct attempts.')
            first[key] = row
    ...
    for key, row in first.items():
        scores = row['scores']
        human_pass = isinstance(row['human_reviewer'], str) and bool(row['human_reviewer'].strip()) and set(scores) == set(DIMENSIONS) and all(type(v) is int and 4 <= v <= 5 for v in scores.values())
        valid_pass = row['status'] == 'pass' and row['delivery'] == 'pass' and row['real_service'] is True and row['clean_session'] is True and human_pass
        if row['status'] == 'pass' and not valid_pass:
            raise ValueError('A pass requires actual service, clean session, Delivery Verification and all five human scores >=4.')
        counts[row['status']] += 1
        if valid_pass:
            passed.append(key)
    ...
    release_pass = len(passed) >= 16 and all_inputs and matrix_pass and not misdelivered and environment_recorded
