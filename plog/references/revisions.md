...
简述理解到的范围后执行，无实质歧义不重复批准。仅文案：保留主体与场景、光色、主要构图，文字变长时只调整必要排版。仅氛围：保留确切文字、主体及主要构图。移动文字：保留文案、声音归属与整体 Look，避开用户指出的手/脸/互动。多项明确要求可合并。

所选旧版的清洁副本是传给图像服务的唯一编辑底图；不要混入原图或其他版本，让服务在不同光色和排版之间重新融合。原图始终是主体与场景真值；使用远程视觉核验时须覆盖该服务的授权：调用前检查所选版本，调用后将原图、所选版本、候选共同对照，不能仅凭与旧版一致接受累积主体或场景漂移。

成功候选需重查主体与场景、中文、目标变化和未请求变化的重要决定。不要求逐像素不变：归一到同一尺寸后，约一两像素的栅格位置差、抗锯齿和细微纹理差异，若没有改变可见文字布局、整体光色、人物结构或关键动作，不单独判失败。“仅换文字”也不等于要求照片文件逐像素相同。未请求且非必要排版调整的字体风格/字号、阅读顺序、位置或整体光色变化仍判失败；裁掉关键主体、实质场景或人物变化独立判失败。用户明确要求逐像素一致或精确尺寸时不能套用这一容差，先说明生成式编辑无法保证的约束；不得静默放宽。失败与停止统一按 [运行约定](runtime.md#失败与清理) 处理。
...
只读重显使用 delivery：校验记录及 PNG 后返回 version_id、完整 image 路径与适用于本地界面的 markdown，预览和下载指向同版；不需要原图或外发授权，不生成、不写记录。路径中的空格、括号及特殊字符由工具处理。本地界面保留返回的 Markdown；远程聊天通过 [宿主交付](agents.md#交付) 发送相同 PNG 文件。append 的命令行返回额外包含 delivery，但记录 schema 与 Python append 返回的版本结构不变。若文件已保存而链接展示中断，核对记录后用 delivery 重显，不为补链接追加版本。

PD_SOURCE_SHA256 必须是本次生成前 inspect 返回的原图校验，不能在生成后重新取值绕过变化检测。首次 append 省略 --version；修改省略则基于当前最新接受版本。append 仅供已完成视觉核验的候选使用，脚本本身不判定场景、人物或文字正确。PD_DETAILS 是本次私密临时区的 JSON，成功/失败后清理，只含：
