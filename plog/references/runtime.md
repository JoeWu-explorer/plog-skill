...
自检真实写入/读取 JPEG、PNG、WebP、EXIF 方向与无元数据 PNG，再清理探测文件；HEIF 另作编解码探测。自检不联网、不做收费生成，host_image_service=unverified 是正常的独立状态。HTML 图册的本地导出按 [多图成册](albums.md) 执行，不要求图片编辑服务。单张 Plog 完整创作还需通过 [Agent 接入](agents.md) 确认读图、图片编辑、对照分析和当前会话文件交付。可用宿主原生工具或已配置的兼容 API；本地自检不证明远程服务可用。
...
成功、失败或停止均清理本次自建暂存源副本、细节 JSON、对照图及预览衍生物；保留用户原图、历史成图和宿主拥有的文件。私密照片、文字、提示词和审阅细节留在用户指定的本地私密位置，不写入技能目录、项目仓库、公共附件或遥测。持久内容为成图和最小版本记录；图册分支另保留 HTML 与最小图册稿；不保存原图副本、永久授权、源元数据、身份档案、敏感推断或全量对话。
